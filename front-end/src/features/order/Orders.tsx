...
import { deleteAddressAsync, getAddressesAsync, postAddressAsync, selectAddress } from "../shipping/shippingSlice";
...
const Orders = () => {
  ...
  const handleOrderSubmit = async (event: any) => {
    ...
    localStorage.setItem("userAddress", JSON.stringify(addresses.find(a => a.id === selectedAddress)));
  ...
  const handleSubmit = async (event: any) => {
    ...
    // Save the address details to local storage
    localStorage.setItem("newAddress", JSON.stringify(shippingData));
  ...
  useEffect(() => {
    ...
    if (storedAddress) {
      ...
      setSelectedAddress(parsedAddress.id);
    }
  }, []);
