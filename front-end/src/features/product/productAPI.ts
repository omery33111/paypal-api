...
export function getPagedProducts(page: number) {
  return new Promise<{ data: Product[] }>((resolve) =>
    axios.get(`${productURL}/get_products/${page}/`).then((res) =>
      resolve({ data: res.data })
    )
  );
}

export function getProductsAmount() {
  return new Promise<{ data: number }>((resolve =>
      axios.get(`${productURL}/products_amount/`).then(res => resolve({ data: res.data }))))}
